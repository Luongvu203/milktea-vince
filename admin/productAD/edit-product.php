...
if (isset($_GET['product_id'])) {
    ...
    // Truy vấn kết hợp bảng products và product_sizes
    $sql = "SELECT p.product_name, p.category_id, p.product_img, ps.product_price, p.product_des
            FROM products p
    ...
    if ($result->num_rows > 0) {
        ...
        $product_price = $row['product_price'];  // Lấy giá từ bảng product_sizes
...
<body>
    <fieldset class="container-add-new-product">
        ...
        <form class="add-new-form" action="" method="POST" enctype="multipart/form-data">
            ...
            <section class="enter-product-price-by-size">
        <label class="label-form-add" for="">Giá Theo Kích Thước:</label>
            <div class="container-input-form">
        <?php
        $sqlSizes = "SELECT * FROM sizes";
        $resultSizes = mysqli_query($conn, $sqlSizes);
        ...
        while ($row = mysqli_fetch_assoc($resultSizes)) {
            ...
            // Lấy giá sản phẩm theo kích thước
            $sqlPrice = "SELECT product_price FROM product_sizes WHERE product_id = '$product_id' AND size_id = '$sizeId'";
            $resultPrice = $conn->query($sqlPrice);
            $sizePrice = ($resultPrice->num_rows > 0) ? $resultPrice->fetch_assoc()['product_price'] : '';

            echo "<label class='label-size' for='size_price_$sizeId'>$sizeName</label>";
            echo "<input class='product-price-size' type='text' name='size_prices[$sizeId]' id='size_price_$sizeId' value='$sizePrice'>";
            }
        ?>
            </div>
            </section>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Xử lý cập nhật thông tin sản phẩm cơ bản
    ...
    // Xử lý giá theo kích thước
    if (isset($_POST['size_prices'])) {
        foreach ($_POST['size_prices'] as $sizeId => $price) {
            // Loại bỏ ký tự không phải là số, dấu phân cách và đơn vị "đ"
            $price = preg_replace('/[^0-9,.đ]/', '', $price); // Giữ lại dấu phẩy, dấu chấm và đơn vị "đ"

            // Lưu giá trị nguyên vẹn (bao gồm "đ")
            if (!empty($price)) {
                // Cập nhật hoặc thêm giá nếu giá trị hợp lệ
                $sqlCheck = "SELECT * FROM product_sizes WHERE product_id = '$product_id' AND size_id = '$sizeId'";
                $resultCheck = $conn->query($sqlCheck);

                if ($resultCheck->num_rows > 0) {
                    $sqlUpdatePrice = "UPDATE product_sizes SET product_price = '$price' WHERE product_id = '$product_id' AND size_id = '$sizeId'";
                    $conn->query($sqlUpdatePrice);
                } else {
                    $sqlInsertPrice = "INSERT INTO product_sizes (product_id, size_id, product_price) VALUES ('$product_id', '$sizeId', '$price')";
                    $conn->query($sqlInsertPrice);
                }
            } else {
                // Nếu giá bằng 0 hoặc để trống, xóa hàng khỏi bảng product_sizes
                $sqlDeletePrice = "DELETE FROM product_sizes WHERE product_id = '$product_id' AND size_id = '$sizeId'";
                $conn->query($sqlDeletePrice);
            }
        }
    }
}
